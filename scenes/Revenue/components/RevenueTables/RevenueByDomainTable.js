...
const RevenueByDomainTable = () => {
  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <Link
          href={`/revenue?by=date&from=${moment(
            row.entryTimeStamp
          ).format("YYYY-MM-DD")}&to=${moment(row.entryTimeStamp).format(
            "YYYY-MM-DD"
          )}`}
          prefetch={false}
        >
          <a>{moment(row.entryTimeStamp).format("YYYY-MM-DD")}</a>
        </Link>
      ),
    },
    {
  ...
  const formattedData = React.useMemo(() => {
    return revenue.map((item) => {
      return {
        Date: moment(item.entryTimeStamp).format("YYYY-MM-DD"),
  ...
  return (
    <>
      ...
      <DataTable
        ...
        data={revenue}
        noHeader={true}
        defaultSortField="entryTimeStamp"
...
export default RevenueByDomainTable;
